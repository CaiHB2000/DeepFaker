#!/usr/bin/env python
"""Generate weak-teacher configs for multiple seeds (WeFEND)."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Create multi-seed weak-teacher configs for WeFEND.")
    parser.add_argument(
        "--base",
        default="dynamic_distill/configs/wefend_teacher_match_refine/wefend_longtrain_pos_balanced.yaml",
    )
    parser.add_argument("--out-dir", default="dynamic_distill/configs/wefend_teacher_match_weakteacher_multi")
    parser.add_argument("--run-script", default="scripts/run_wefend_teacher_match_weakteacher_multi.sh")
    ...
    run_lines = ["#!/usr/bin/env bash", "set -e", "mkdir -p logs/wefend_teacher_match_weakteacher_multi"]
    ...
    for cfg_path in generated_paths:
        ...
        for seed in args.seeds:
            ...
            log_path = f"logs/wefend_teacher_match_weakteacher_multi/{stem}_seed{seed}.log"
            cmd = (
            ...
            if job_idx % len(GPU_LIST) == 0 and job_idx < total_jobs:
                ...
                run_lines.append("echo '--- weak-teacher multi batch slice complete ---'")
    run_lines.append("wait")
    run_lines.append("echo 'All weak-teacher multi jobs finished.'")
