...
from dynamic_distill.src.models import SafeFusionModel, TextEncoder, VisionEncoder
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train SAFE-style fusion baseline.")
...
def build_collate_fn(
    ...
    def collate(batch: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        meta = {"ids": [sample.get("id") for sample in batch]}
...
@torch.no_grad()
def evaluate(model: SafeFusionModel, loader, device: torch.device) -> Dict[str, float]:
    if loader is None:
        return {}
...
def main() -> None:
    ...
    if not run_name:
        run_name = f"{config['data']['type']}_safe_{timestamp}"
    if args.seed is not None:
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, batch in enumerate(train_iterable, start=1):
            ...
            if batch_idx % log_every == 0:
                print(f"[epoch {epoch+1}/{epochs}] step={batch_idx} loss={loss.item():.4f}", flush=True)
        ...
        val_metrics = evaluate(model, data_module.val, device)
    ...
    test_metrics = evaluate(model, data_module.test, device)
    print(
