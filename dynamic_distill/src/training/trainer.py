...
from ..modules.reliability import ReliabilityEstimator
...
class TrainerConfig:
    ...
    # RaDMPD reliability module
    reliability_enabled: bool = False
    reliability_lambda_mix: float = 0.0
    reliability_lambda_gate: float = 0.0
    reliability_lambda_rel: float = 0.0
    reliability_event_dim: int = 32
    reliability_hidden_dim: int = 64
    reliability_extra_dim: int = 2  # mean conf, mean disagree
    reliability_num_events: int = 5000
...
class DynamicDistillationTrainer:
    ...
    def __init__(
        ...
        self.event_reliability_stats = defaultdict(lambda: {"seen": 0, "conf_sum": 0.0, "disagree_sum": 0.0})
        ...
        self.reliability_module: Optional[ReliabilityEstimator] = None
        if self.config.reliability_enabled:
            self.reliability_module = ReliabilityEstimator(
                num_modalities=3,
                num_events=self.config.reliability_num_events,
                event_dim=self.config.reliability_event_dim,
                hidden_dim=self.config.reliability_hidden_dim,
                extra_dim=self.config.reliability_extra_dim,
            ).to(self.device)
    ...
    def train_step(self, batch: Dict[str, Any]) -> Dict[str, float]:
        ...
        with torch.no_grad():
            ...
            # modality disagreement (binary: prob of class 1 diff)
            prob_text_fake = probs_text_full[:, 1] if probs_text_full.shape[1] > 1 else probs_text_full[:, 0]
            prob_img_fake = probs_vision_full[:, 1] if probs_vision_full.shape[1] > 1 else probs_vision_full[:, 0]
            disagree_vals = torch.abs(prob_text_fake - prob_img_fake)
            ...
            if (
                ...
                for idx, event_id in enumerate(event_ids_batch):
                    ...
                    rel_stats = self.event_reliability_stats[eid]
                    rel_stats["seen"] += 1
                    rel_stats["conf_sum"] += float(fusion_conf[idx].item())
                    rel_stats["disagree_sum"] += float(disagree_vals[idx].item())
            ...
            if meta is not None and isinstance(meta, dict) and "event_ids" in meta:
                ...
                for idx_sample, event_id in enumerate(event_ids_batch):
                    ...
                    # reliability stats (mean conf / disagreement)
                    if self.config.reliability_enabled:
                        rel_stats = self.event_reliability_stats[eid]
                        seen_rel = max(rel_stats["seen"], 1)
                        conf_val = rel_stats["conf_sum"] / seen_rel
                        disagree_val = rel_stats["disagree_sum"] / seen_rel
                        if "event_conf_mean" not in meta:
                            meta["event_conf_mean"] = torch.zeros_like(labels, dtype=torch.float32, device=labels.device)
                            meta["event_disagree_mean"] = torch.zeros_like(labels, dtype=torch.float32, device=labels.device)
                        meta["event_conf_mean"][idx_sample] = conf_val
                        meta["event_disagree_mean"][idx_sample] = disagree_val
            ...
            # RaDMPD reliability mixture and weights
            reliability_weights = None
            rel_loss = torch.tensor(0.0, device=device)
            if (
                self.config.reliability_enabled
                and self.reliability_module is not None
                and meta is not None
                and isinstance(meta, dict)
                and "event_ids" in meta
            ):
                event_ids_batch = meta["event_ids"].to(labels.device)
                # extra feats: mean fusion conf, mean text-img disagreement
                conf_feat = meta.get("event_conf_mean")
                disagree_feat = meta.get("event_disagree_mean")
                if conf_feat is None:
                    conf_feat = torch.zeros_like(labels, dtype=torch.float32, device=labels.device)
                if disagree_feat is None:
                    disagree_feat = torch.zeros_like(labels, dtype=torch.float32, device=labels.device)
                extra_feats = torch.stack([conf_feat, disagree_feat], dim=-1)
                mod_text = torch.full_like(event_ids_batch, 0)
                mod_img = torch.full_like(event_ids_batch, 1)
                mod_fuse = torch.full_like(event_ids_batch, 2)
                pi_text = self.reliability_module(mod_text, event_ids_batch, extra_feats)
                pi_img = self.reliability_module(mod_img, event_ids_batch, extra_feats)
                pi_fuse = self.reliability_module(mod_fuse, event_ids_batch, extra_feats)
                pi_stack = torch.stack([pi_text, pi_img, pi_fuse], dim=-1)
                alpha = pi_stack / (pi_stack.sum(dim=-1, keepdim=True) + 1e-8)
                lambda_mix = max(self.config.reliability_lambda_mix, 0.0)
                mix_logits = (
                    (1 - lambda_mix) * fusion_teacher_logits
                    + lambda_mix
                    * (
                        alpha[:, 0].unsqueeze(-1) * teacher_logits_text
                        + alpha[:, 1].unsqueeze(-1) * teacher_logits_vision
                        + alpha[:, 2].unsqueeze(-1) * fusion_teacher_logits
                    )
                )
                if lambda_mix > 0:
                    teacher_logits_text = mix_logits
                    teacher_logits_vision = mix_logits
                # soft gate weight
                q_probs = F.softmax(mix_logits.detach(), dim=-1)
                entropy = -(q_probs * (torch.log(q_probs + 1e-8))).sum(dim=-1)
                s_i = 1.0 - entropy / math.log(q_probs.shape[-1])
                r_agg = (alpha * torch.stack([pi_text, pi_img, pi_fuse], dim=-1)).sum(dim=-1)
                lambda_gate = max(self.config.reliability_lambda_gate, 0.0)
                if eligible_mask is not None:
                    g_old = eligible_mask.float()
                else:
                    g_old = torch.ones_like(s_i, device=device)
                reliability_weights = g_old * ((1 - lambda_gate) + lambda_gate * s_i * r_agg)
                reliability_weights = reliability_weights.clamp(min=0.0)
                # reliability regularization
                if self.config.reliability_lambda_rel > 0 and labels is not None:
                    t_corr_text = (teacher_logits_text.argmax(dim=-1) == labels).float()
                    t_corr_img = (teacher_logits_vision.argmax(dim=-1) == labels).float()
                    t_corr_fuse = (fusion_teacher_logits.argmax(dim=-1) == labels).float()
                    rel_loss = (
                        F.binary_cross_entropy(pi_text, t_corr_text)
                        + F.binary_cross_entropy(pi_img, t_corr_img)
                        + F.binary_cross_entropy(pi_fuse, t_corr_fuse)
                    ) / 3.0
            ...
            if (
                ...
                override_weights = reliability_weights
        ...
        total_loss = (
            ...
            + self.config.reliability_lambda_rel * rel_loss
        )
        if trace_this:
            print(
