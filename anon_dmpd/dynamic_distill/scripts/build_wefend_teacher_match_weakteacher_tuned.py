#!/usr/bin/env python
"""Hand-tuned weak-teacher configs for WeFEND with multi-seed support."""
...
STRATEGIES: List[Strategy] = [
    (
        "weakteacher_tuned_high",
        {
            ("distillation", "event_layer", "enabled"): True,
            ("distillation", "event_layer", "metric"): "min",
            ("distillation", "event_layer", "min_seen"): 6,
            ("distillation", "event_layer", "high_threshold"): 0.935,
            ("distillation", "event_layer", "mid_threshold"): 0.89,
            ...
            ("distillation", "weak_teacher", "lambda"): 0.13,
            ...
            ("distillation", "weak_teacher", "temperature"): 2.24,
    ...
    (
        "weakteacher_tuned_mid",
        {
            ...
            ("distillation", "weak_teacher", "temperature"): 2.22,
    ...
    (
        "weakteacher_tuned_softquota",
        {
            ...
            ("distillation", "soft_quota", "max_frac"): 0.07,
            ("distillation", "soft_quota", "per_event_cap_frac"): 0.12,
        },
    ),
]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Create tuned weak-teacher configs for WeFEND.")
    parser.add_argument(
        "--base",
        default="dynamic_distill/configs/wefend_teacher_match_refine/wefend_longtrain_pos_balanced.yaml",
    )
    parser.add_argument("--out-dir", default="dynamic_distill/configs/wefend_teacher_match_weakteacher_tuned")
    parser.add_argument("--run-script", default="scripts/run_wefend_teacher_match_weakteacher_tuned.sh")
    ...
    run_lines = ["#!/usr/bin/env bash", "set -e", "mkdir -p logs/wefend_teacher_match_weakteacher_tuned"]
    ...
    for cfg_path in generated_paths:
        ...
        for seed in args.seeds:
            ...
            log_path = f"logs/wefend_teacher_match_weakteacher_tuned/{stem}_seed{seed}.log"
            cmd = (
            ...
            if job_idx % len(GPU_LIST) == 0 and job_idx < total_jobs:
                ...
                run_lines.append("echo '--- weakteacher tuned slice complete ---'")
    run_lines.append("wait")
    run_lines.append("echo 'All weakteacher tuned jobs finished.'")
