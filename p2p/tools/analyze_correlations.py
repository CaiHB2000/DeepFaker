...
def main() -> None:
    ...
    top_preview = correlations_sorted[:5]
    if top_preview:
        print("[analysis] top correlations (Spearman):")
        for item in top_preview:
            print(
                f"  {item['feature']} vs {item['detector_metric']}: "
                f"spearman={fmt_float(item['spearman_r'])} (n={item['n']})"
            )
