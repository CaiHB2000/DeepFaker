...
def _abs_path(base: str, maybe_rel: str) -> str:
    # 支持像 '.'、'./datasets/lmdb' 这样的相对写法
...
class DeepfakeAbstractBaseDataset(data.Dataset):
    ...
    def _resolve_disk_path(self, file_path: str) -> str:
        """根据 config['rgb_dir'] 与传入的 file_path 决定最终磁盘路径"""
        ...
        # 如果 p 里已经包含 rgb_dir，则避免重复拼接
        rgb_dir = self.config.get('rgb_dir', '')
        if rgb_dir:
            rgb_dir = self._sanitize_path(rgb_dir)
            # 尝试：p 相对路径 -> 拼到 rgb_dir
            cand1 = os.path.normpath(os.path.join(rgb_dir, p))
            if os.path.exists(cand1):
                return cand1
            # 如果 p 已经带上了 rgb_dir 的末段（例如 datasets/rgb/...），再试去重拼接
            # 去掉可能的 rgb_dir 前缀后再拼
            if p.startswith(rgb_dir):
                p2 = p[len(rgb_dir):].lstrip(os.sep).lstrip('/')
                cand2 = os.path.normpath(os.path.join(rgb_dir, p2))
                if os.path.exists(cand2):
                    return cand2

        # 最后再试：相对当前工作目录
        cand3 = os.path.abspath(p)
        if os.path.exists(cand3):
            return cand3

        # 全都找不到，抛错并给出调试信息
        raise FileNotFoundError(
            f"[load_rgb] File not found.\n"
            f" - given: {file_path}\n"
            f" - sanitized: {p}\n"
            f" - rgb_dir: {self.config.get('rgb_dir', '')}\n"
            f" - tried: {locals().get('cand1', None)}, {locals().get('cand2', None)}, {cand3}"
        )



    def _infer_roots_from_yaml(self):
        """
        兼容 yaml:
          rgb_dir: '.'
          lmdb_dir: './datasets/lmdb'
        推断出：
          self.rgb_root = <repo>/datasets/rgb
          self.lmdb_root = <repo>/datasets/lmdb
        """
        # 以当前工作目录为 repo 根（你一般就是在仓库根跑脚本）
        repo_root = os.getcwd()
        rgb_dir = self.config.get('rgb_dir', '.')
        lmdb_dir = self.config.get('lmdb_dir', './datasets/lmdb')

        # 如果 rgb_dir 是 '.'，我们把真正的 rgb_root 解析到 <repo>/datasets/rgb
        # 如果用户以后把 rgb_dir 配成绝对路径，也能兼容
        rgb_abs = _abs_path(repo_root, rgb_dir)
        # 优先识别出 '<repo>/datasets/rgb' 这一层
        if os.path.basename(rgb_abs) == 'rgb' and os.path.basename(os.path.dirname(rgb_abs)) == 'datasets':
            self.rgb_root = rgb_abs
        else:
            # 常见情况：rgb_dir='.'，那我们认为 rgb 在 <repo>/datasets/rgb
            guess = os.path.join(repo_root, 'datasets', 'rgb')
            self.rgb_root = guess if os.path.isdir(guess) else rgb_abs

        self.lmdb_root = _abs_path(repo_root, lmdb_dir)
        os.makedirs(self.lmdb_root, exist_ok=True)

    def _infer_dataset_name(self, abs_path: str) -> str:
        """
        从绝对路径里，取出 rgb_root 之后的第一段目录作为数据集名：
          <rgb_root>/<ds_name>/...
        例如：.../datasets/rgb/Celeb-DF-v2/Celeb-synthesis/frames/xxx.png
               -> ds_name = 'Celeb-DF-v2'
        """
        ap = _sanitize_path(abs_path)
        root = _sanitize_path(self.rgb_root)
        if root in ap:
            tail = ap.split(root.rstrip('/'), 1)[1].lstrip('/')
            return tail.split('/', 1)[0] if '/' in tail else tail
        return ''

    def _get_lmdb_env(self, ds_name: str):
        """
        为每个数据集单独打开一个 LMDB：
          <lmdb_root>/<ds_name>_lmdb
        例如：datasets/lmdb/Celeb-DF-v2_lmdb
        """
        if not hasattr(self, '_lmdb_envs'):
            self._lmdb_envs = {}
        if ds_name in self._lmdb_envs:
            return self._lmdb_envs[ds_name]

        lmdb_path = os.path.join(self.lmdb_root, f"{ds_name}_lmdb")
        # 容错：如果目录名已经是 *_lmdb 就不要重复加
        if not os.path.isdir(lmdb_path) and ds_name.endswith('_lmdb'):
            lmdb_path = os.path.join(self.lmdb_root, ds_name)

        env = lmdb.open(lmdb_path, readonly=True, lock=False, readahead=False)
        self._lmdb_envs[ds_name] = env
        return env
    ...
    def load_rgb(self, file_path):
        """
        Load an RGB image from a file path (or LMDB) and resize it to a specified resolution.
        """
        ...
        if not self.lmdb:
            ...
            if img is None:
                # OpenCV 读不到通常是路径仍不对或文件损坏
                raise ValueError(f"[load_rgb] Loaded image is None: {disk_path}")
        else:
            abs_p = os.path.abspath(_sanitize_path(file_path))
            # 1) 推断 ds_name（如 'Celeb-DF-v2'）
            ds_name = self._infer_dataset_name(abs_p)
            if not ds_name:
                raise RuntimeError(f"[LMDB] cannot infer dataset name from path: {abs_p}\n"
                                   f"rgb_root={self.rgb_root}")

            # 2) 定位该数据集的 LMDB
            env = self._get_lmdb_env(ds_name)

            # 3) 生成 LMDB key（与你的库一致：从数据集下一层开始）
            #    先算相对 <rgb_root>/<ds_name> 的路径：
            ds_root = os.path.join(self.rgb_root, ds_name)
            rel_from_ds = os.path.relpath(abs_p, ds_root).replace('\\', '/')
            # 例如：'Celeb-synthesis/frames/id6_id21_0006/037.png'
            ...
            add(rel_from_ds)  # 首选：与你当前 LMDB 完全一致的键
            # 兜底：如果库里误存了包含 ds_name 的相对路径
            add(f"{ds_name}/{rel_from_ds}")
            # 兜底：去掉可能误带的 'datasets/rgb/'
            if rel_from_ds.startswith('datasets/rgb/'):
                add(rel_from_ds[len('datasets/rgb/'):])

            with env.begin(write=False) as txn:
                image_bin, hit_key = None, None
                for k in candidates:
                    image_bin = txn.get(k.encode())
                    if image_bin is not None:
                        hit_key = k;
                        break

                if image_bin is None:
                    raise FileNotFoundError(f"[LMDB] miss key. given={file_path}  tried={candidates}")

                image_buf = np.frombuffer(image_bin, dtype=np.uint8)
                img = cv2.imdecode(image_buf, cv2.IMREAD_COLOR)
                if img is None:
                    raise ValueError(f"[LMDB] cv2.imdecode failed. key={hit_key}")
